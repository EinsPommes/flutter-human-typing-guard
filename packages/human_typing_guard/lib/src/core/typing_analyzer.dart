...
class TypingAnalyzer {
  ...
  TypingFeatures _calculateFeatures() {
    ...
    // Burstiness: (σ - μ) / (σ + μ)
    final burstiness = ikiStd > 0
        ? (ikiStd - ikiMean) / (ikiStd + ikiMean)
        : 0.0;
    ...
    final backspacePer100 = totalKeystrokes > 0
        ? (backspaceCount / totalKeystrokes) * 100
        : 0.0;

    final pasteEvents = _events.where((e) => e.isPaste).length;
    final jitterMad = _calculateJitterMad(ikis);

    final outlierCount = ikis
        .where((iki) => (iki - ikiMean).abs() > 3 * ikiStd)
        .length;
  ...
  List<double> _calculateInterKeyIntervals() {
    ...
    for (int i = 1; i < regularEvents.length; i++) {
      final interval =
          regularEvents[i].timestamp - regularEvents[i - 1].timestamp;
  ...
  double _normalizeBackspace(double backspacePer100) {
    ...
    if (backspacePer100 >= 2.0 && backspacePer100 <= 10.0)
      return 1.0; // optimal
